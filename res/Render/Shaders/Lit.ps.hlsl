...
float3 BurleyDiffuse(float3 N, float3 V, float3 L, float roughness)
{
    ...
    return lightScatter * viewScatter / 3.14159265;
}
...
float3 CalculateAdvancedDirectionnalTexture(float3 worldPosition, float3 basenormal, float3 tangent, float2 uv, float3 sampleAlbedo, float3 sampleAmbient, float sampleRoughness, float sampleMetallic, LightData light)
{
    ...
    float NdotL_final = max(dot(normal, lightDir), 0.0f); // used only for light scaling, Burley handles diffuse shape
    ...
    float3 diffuse = kD * sampleAlbedo * BurleyDiffuse(normal, viewDir, lightDir, sampleRoughness);
