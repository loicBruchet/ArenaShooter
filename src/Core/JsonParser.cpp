...
///////////////////////////////////////////////////////////////////////////////
std::vector<JsonObject*>* JsonParser::Deserialize(String path)
{
	std::vector<JsonObject*>* jsonVector = new std::vector<JsonObject*>();

	File* pFile = RunFile(path, File::READ_ONLY);
	
	while (pFile->GetCursor() < pFile->GetSize())
	{
		jsonVector->push_back(ParseObject(pFile, nullptr));
	}
