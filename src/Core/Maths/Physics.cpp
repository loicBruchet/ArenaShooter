...
{
    std::vector<Vector3f32> listOfPoint = GetListFromMinMax(box.min, box.max);
...
std::vector<Vector3f32> Physics::GetListFromMinMax(Vector3f32 const& min, Vector3f32 const& max)
{
    std::vector<Vector3f32> list;

    list.push_back({ min.x, min.y, min.z });
    list.push_back({ max.x, max.y, max.z });
    list.push_back({ max.x, max.y, min.z });
    list.push_back({ max.x, min.y, min.z });
    list.push_back({ min.x, max.y, min.z });
    list.push_back({ max.x, min.y, max.z });
    list.push_back({ min.x, max.y, max.z });
    list.push_back({ min.x, min.y, max.z });
...
{
	std::vector<Vector2f32> listOfPoint = GetListFromMinMax(box.min, box.max);
...
std::vector<Vector2f32> Physics::GetListFromMinMax(Vector2f32 const& min, Vector2f32 const& max)
{
	std::vector<Vector2f32> list;
	list.push_back({ min.x, min.y });
	list.push_back({ max.x, max.y });
	list.push_back({ max.x, min.y });
	list.push_back({ min.x, max.y });
	return list;
}
...
std::vector<Vector3f32> Physics::GetCorners(Box const& a)
{
    std::vector<Vector3f32> cornersA;

    Vector3f32 extentA = (a.max - a.min) * 0.5f;

    cornersA.push_back(a.center + a.axisX * extentA.x + a.axisY * extentA.y + a.axisZ * extentA.z);
    cornersA.push_back(a.center - a.axisX * extentA.x + a.axisY * extentA.y + a.axisZ * extentA.z);
    cornersA.push_back(a.center + a.axisX * extentA.x - a.axisY * extentA.y + a.axisZ * extentA.z);
    cornersA.push_back(a.center + a.axisX * extentA.x + a.axisY * extentA.y - a.axisZ * extentA.z);
    cornersA.push_back(a.center - a.axisX * extentA.x - a.axisY * extentA.y + a.axisZ * extentA.z);
...
CollideResult Physics::IntersectBoxBox(Box const& a, Box const& b)
{
    ...
    std::vector<Vector3f32> cornersA = GetCorners(a);
    std::vector<Vector3f32> cornersB = GetCorners(b);

    std::vector<Vector3f32> penetratingPoints;
    ...
    for (const Vector3f32& pt : cornersA)
    {
        if (Abs(contactPlane.DistanceTo(pt)) < 0.01f) penetratingPoints.push_back(pt);
    }
