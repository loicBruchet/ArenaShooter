...
public : 
	...
	static std::vector<Vector3f32> GetListFromMinMax(Vector3f32 const& min, Vector3f32 const& max);
	//2D
	static void UpdateBoundingBox(Square& const box, Matrix const& world, Vector2f32 const& scale);
	static std::vector<Vector2f32> GetListFromMinMax(Vector2f32 const& min, Vector2f32 const& max);
	...
	static std::vector<Vector3f32> GetCorners(Box const& a);
