...
Quaternion* Quaternion::operator*=(Quaternion const& o)
	...
	gce::Vector3f32 Quaternion::ToEulerAngles() const {
		float32 ysqr = pdata[1] * pdata[1];
		// roll (x-axis rotation)
		float32 t0 = +2.0f * (pdata[3] * pdata[0] + pdata[1] * pdata[2]);
		float32 t1 = +1.0f - 2.0f * (pdata[0] * pdata[0] + ysqr);
		float32 roll = atan2f(t0, t1);
		// pitch (y-axis rotation)
		float32 t2 = +2.0f * (pdata[3] * pdata[1] - pdata[2] * pdata[0]);
		t2 = t2 > 1.0f ? 1.0f : t2;
		t2 = t2 < -1.0f ? -1.0f : t2;
		float32 pitch = asinf(t2);
		// yaw (z-axis rotation)
		float32 t3 = +2.0f * (pdata[3] * pdata[2] + pdata[0] * pdata[1]);
		float32 t4 = +1.0f - 2.0f * (ysqr + pdata[2] * pdata[2]);
		float32 yaw = atan2f(t3, t4);
		return Vector3f32(roll, pitch, yaw);
	}

	float32 Quaternion::Dot(Quaternion const& other) const
	{
		__m128 Q1 = _mm_load_ps(pdata);
		__m128 Q2 = _mm_load_ps(other.pdata);
		// Perform the dot product on x,y,z and w
		__m128 vDot = _mm_mul_ps(Q1, Q2);
		// vTemp has z and w
		__m128 vTemp = _mm_shuffle_ps(vDot, vDot, _MM_SHUFFLE(3, 2, 3, 2));
		// x+z, y+w
		vDot = _mm_add_ps(vDot, vTemp);
		// x+z,x+z,x+z,y+w
		vDot = _mm_shuffle_ps(vDot, vDot, _MM_SHUFFLE(1, 0, 0, 0));
		// ??,??,y+w,y+w
		vTemp = _mm_shuffle_ps(vTemp, vDot, _MM_SHUFFLE(3, 3, 0, 0));
		// ??,??,x+z+y+w,??
		vDot = _mm_add_ps(vDot, vTemp);
		return vDot.m128_f32[2];
	}

	void Quaternion::LookAt(gce::Vector3f32 const& direction, gce::Vector3f32 const& up)
	{
		Vector3f32 f = direction.Normalize();
		Vector3f32 r = up.CrossProduct(f).Normalize();
		Vector3f32 u = f.CrossProduct(r);
		Matrix mat;
		mat.m_matrix[0][0] = r.x; mat.m_matrix[0][1] = r.y; mat.m_matrix[0][2] = r.z; mat.m_matrix[0][3] = 0;
		mat.m_matrix[1][0] = u.x; mat.m_matrix[1][1] = u.y; mat.m_matrix[1][2] = u.z; mat.m_matrix[1][3] = 0;
		mat.m_matrix[2][0] = f.x; mat.m_matrix[2][1] = f.y; mat.m_matrix[2][2] = f.z; mat.m_matrix[2][3] = 0;
		mat.m_matrix[3][0] = 0;   mat.m_matrix[3][1] = 0;   mat.m_matrix[3][2] = 0;   mat.m_matrix[3][3] = 1;
		SetRotationMatrix(mat);
	}

	void Quaternion::Lerp(Quaternion const& target, float32 t)
	{
		Quaternion result;
		result.pdata[0] = pdata[0] + t * (target.pdata[0] - pdata[0]);
		result.pdata[1] = pdata[1] + t * (target.pdata[1] - pdata[1]);
		result.pdata[2] = pdata[2] + t * (target.pdata[2] - pdata[2]);
		result.pdata[3] = pdata[3] + t * (target.pdata[3] - pdata[3]);
