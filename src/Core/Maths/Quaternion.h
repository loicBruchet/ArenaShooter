...
namespace gce
{
	...
	struct Quaternion
	{
		...
		Vector3f32 ToEulerAngles() const;

		float32 Dot(Quaternion const& other) const;

		void LookAt(Vector3f32 const& direction, Vector3f32 const& up = Vector3f32(0.0f, 1.0f, 0.0f));
		void Lerp(Quaternion const& target, float32 t);
