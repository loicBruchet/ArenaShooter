...
namespace gce {
	...
	private:
		...
		std::vector<ResultInstance*> m_resultInstances;
		std::vector<FunctionInstance*> m_functionInstances;
	...
	private:
		inline static std::vector<Tween<void>*> s_tweens;
	...
	{
		m_resultInstances.push_back(new ResultInstance{ duration, 0.0f, &result, reverse });
	}

	template <typename T> template <typename C>
	void Tween<T>::StartDuration(float32 const duration, Function<void(C::*)(T const&)> const& resultFunction, bool const reverse)
	{
		m_functionInstances.push_back(new FunctionInstance{ duration, 0.0f, new Function<void(C::*)(T const&)>(resultFunction), reverse });
	}
...
//     inline static std::vector<TweenInstance*> s_activeTweens;
...
// 		std::vector<std::vector<Tween::TweenInstance*>> m_tweens;
// 	};
//
// 	SequenceInstance* m_currentSequence = nullptr;
// 	inline static std::vector<SequenceInstance*> s_activeSequences;
// 	friend class Tween;
// };
//
//
